...
def _crawl_hot_list(session: requests.Session) -> Dict[str, int]:
    """从 ZOL 热门手机排行动态抓取型号 ID 及页面名次。"""
    url = 'https://top.zol.com.cn/compositor/57/cell_phone.html'
    ranks: Dict[str, int] = {}
    try:
        human_delay("ZOL 热门手机排行")
        resp = session.get(url, timeout=REQUEST_TIMEOUT)
        if resp.status_code != 200:
            logger.warning(f"ZOL 热门手机排行返回 HTTP {resp.status_code}，保持列表原始顺序")
            return ranks
        resp.encoding = 'gbk'
        for match in re.finditer(r'cell_phone/index(\d+)\.shtml', resp.text):
            phone_id = match.group(1)
            if phone_id not in ranks:
                ranks[phone_id] = len(ranks)
    except Exception as exc:
        logger.warning(f"ZOL 热门手机排行抓取失败，保持列表原始顺序: {exc}")
        return {}
    logger.info(f"ZOL 热门手机排行共 {len(ranks)} 个有序型号 ID（页面动态获取）")
    return ranks


def _prioritize_hot_models(phones: List[Dict], ranks: Dict[str, int]) -> List[Dict]:
    """榜内严格按页面名次，榜外保持列表扫描的原始顺序。"""
    if not ranks:
        return phones
    source_order = {id(phone): index for index, phone in enumerate(phones)}
    return sorted(
        phones,
        key=lambda phone: (
            (0, ranks[str(phone.get('id'))])
            if str(phone.get('id')) in ranks
            else (1, source_order[id(phone)])
        ),
    )
...
def step1_crawl_list_and_detail():
    ...
    if INCREMENTAL_MODE:
        ...
        hot_ranks = _crawl_hot_list(session)
        all_phones = _prioritize_hot_models(all_phones, hot_ranks)
        if hot_ranks:
            logger.info(f"已按 ZOL 热门手机排行（{len(hot_ranks)} 个有序型号）重排；榜外保持扫描原序，前 5 名: "
